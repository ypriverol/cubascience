...
E changes the **composition** more than the headline: pop end-2025 **8.96M** vs D **8.59M** (still inside the 1.8–2.4M loss envelope). Hidden deaths sum ~+73k over 2022–2025 vs register; births slightly below register. Migration remains dominant. **Important:** E’s \(U_0\) prior is much smaller than D’s, so the higher stock is partly a different baseline prior — not independent vital evidence. E does **not** overturn the emigration-first story.

### Promote?
**No.** Keep D as illustrative central *scenario* (not an identified estimator). Ship E as methods companion + claim-sheet block with `promote: false`. Revisit after (a) full age-schedule from ONEI 3.15, (b) harmonized \(U_0\) with D, (c) weight sensitivity on COVID/ageing, (d) joint posterior rather than sequential medians.
